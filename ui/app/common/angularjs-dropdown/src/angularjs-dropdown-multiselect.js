...
directiveModule.directive('ngDropdownMultiselect', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
    return {
        ...
        template: function(element, attrs) {
            ...
            template += '<button ng-disabled="disabled" type="button" class="dropdown-toggle" ng-class="settings.buttonClasses" ng-click="toggleDropdown()">{{getButtonText() | translate}}&nbsp;<span class="caret"></span></button>';
            ...
            template += '<li ng-if="settings.showCheckAll && settings.selectionLimit !== 1"><a ng-keydown="keyDownLink($event)" data-ng-click="selectAll()" tabindex="-1" id="selectAll">  {{texts.checkAll | translate}}</a>';
            template += '<li ng-if="settings.showUncheckAll"><a ng-keydown="keyDownLink($event)" data-ng-click="deselectAll();" tabindex="-1" id="deselectAll">   {{texts.uncheckAll | translate}}</a></li>';
        ...
        link: function($scope, $element, $attrs) {
            ...
            $scope.texts = {
                checkAll: 'REGISTRATION_ID_CHECK',
                uncheckAll: 'REGISTRATION_ID_UN_CHECK',
                selectionCount: 'checked',
                selectionOf: '/',
                searchPlaceholder: 'Search...',
                buttonDefaultText: 'REGISTRATION_ADD_MORE',
